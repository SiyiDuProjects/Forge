...
- Verification: `node --check server.mjs`, `node --check app.js`, `node --test tests/query_engine.test.mjs` passes with 19 QueryEngine tests including the forge-tool demo path and Codex-selected ProductPlan creation with a delayed thread id, `node --test tests/project_workspace.test.mjs` passes with 9 project-workspace tests, `npm run smoke:codex-live` skips without live opt-in, `node scripts/codex-live-smoke.mjs --run` refuses without explicit external-context ACK, and full `npm run check` passes with 60 tests. The opt-in live smoke now initializes the first ProductPlan through `runtimeProvider: "codex"` before running the modification/generation/revert sequence.
