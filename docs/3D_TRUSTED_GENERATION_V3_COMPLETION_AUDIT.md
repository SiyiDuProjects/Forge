...
| Forge-controlled same-type parts can enter through reviewable project-local specs | Complete | Workspace draft scaffold/spec/promote/select actions and `forge-tool` paths support controlled `component-drafts/<draftId>/descriptor.json`, `sources.md`, and `source-specs.md`; regression coverage proves CLI and Codex-runtime source-spec onboarding through confirmed generation without creating a user-uploaded-parts product surface. |
