...
Connector-position and connector-orientation extraction is similarly conservative. Source text must name an existing connector id from the same-type reference descriptor, such as `connector usb_c position 0, -18, -3 mm`, `connector gpio position x=24 y=12 z=-2 mm`, or `connector usb_c orientation -z`. Forge updates only `connectors[].positionLocalMm` or `connectors[].orientation`; it does not create new connector ids, change connector types, or change `connectors[].mating`. Updated connector anchors still go through the local-position readiness gate before promotion.

External-feature position extraction uses the same boundary. Source text must name an existing external feature id or type, such as `feature button_hole position 1, 0, 3 mm`, `opening usb_c_cutout position ...`, or `开孔 <id> 位置 ...`. Forge updates only `externalFeatures[].positionLocalMm`; it does not create new feature ids, new feature types, or new shell layout rules. Updated feature anchors still go through the local-position readiness gate before promotion.

Keepout/access-volume extraction is also id/type bounded. Source text must name an existing keepout or access-volume id or type from the same-type reference descriptor, such as `keepout button_travel_keepout size 12 x 12 x 9 mm position 0, 0, 6 mm` or `access volume button_wire_access size 12 x 9 x 7 mm position 0, -9, -2 mm`. Forge updates only `sizeMm` and `positionLocalMm`; it does not create new volumes, change `accessVolumes[].connectorId`, or infer service/wire space from arbitrary prose or mesh geometry. Updated volume sizes still go through the preview-solid minimum dimension gate before promotion.

Cable-exit extraction is connector-id bounded. Source text must name a connector id that already has a same-type reference `cableExitDirections[]` entry, such as `cable exit usb_c direction external_rear` or `线缆出口 signal 方向 -y_to_core_board`. Forge updates only the existing `direction` token; it does not create new cable exits, create connectors, alter access volumes, or change mating endpoints. Missing cable exits remain explicit descriptor package feedback rather than guessed routing metadata.
