...
- Forge-authored product conversation logic. Codex owns ordinary chat, clarification, task decisions, and method/tool/interface selection; Forge owns tools, state, validation, artifacts, and guardrails.
...
- Bottom composer for hardware conversation entry and send/stop only. Messages go to the Codex project thread first; Forge does not treat every composer message as ProductPlan input.
...
- Send message to Codex. ProductPlan creation, revision updates, and model generation occur only when Codex calls a Forge tool.
...
- Codex-native conversation UI that sends user text to Codex first and shows an explicit retryable error if Codex/backend runtime is unavailable.
...
- One seeded real backend-generated conversation with confirmed 3D model artifacts; new projects start as blank Codex conversation workspaces with no ProductPlan, revision, GeometrySpec, fallback plan, or generated artifacts until Codex calls `createProductPlan`.
...
- Codex SDK is the default frontend product-task runtime (`runtimeProvider: "codex"`): normal product conversations create or resume the project-bound Codex thread, while Forge remains the bounded persistence/tool layer for ProductPlan, revisions, proposals, GeometrySpec, validation, and artifacts. Product UI conversation must not fall back to Forge-authored chat responses; if Codex is unavailable, show an error and preserve the draft input. The deterministic local Forge adapter (`runtimeProvider: "mock"`) remains an explicit test/tool-runtime path for existing ProductPlan action tests, not the default product conversation brain. OpenAI Responses remains available through the Forge QueryEngine path.
...
1. User sends a message into a clean Codex conversation thread.
2. Codex answers, discusses, asks clarifying questions, or decides to call a Forge tool.
3. Only when Codex calls `createProductPlan`, Forge creates the first ProductPlan/revision; ordinary conversation does not create product state.
4. Parser extracts product type, 3D printed enclosure finish, screen size, and options from the explicit ProductPlan request.
5. Module matcher builds a parts list (BOM) from known modules.
6. Risk-limit gate flags camera and battery for human review and blocks motion structures from the standard path.
7. Quote estimator creates hardware/build/manufacturing-check cost bands.
8. Later Codex tool calls can create pending ProductPlan revisions from explicit user changes.
9. The model-generation job locks a `GeometrySpec`, validates geometry, and waits for explicit generation confirmation before writing GLB/STL/STEP.
10. Generation jobs attach a prototype structure preview (3D), electronics layout, quote assumptions, and validation report to the revision.
11. User enters name and email, then clicks `提交审核下单`.
12. The app writes a local human review packet; no payment or manufacturing starts.
...
- Keep component availability Forge-controlled. Users should choose from Forge-supported parts and finishes; project-local descriptor draft/spec tooling is for Forge operators, Codex product agents, or vetted supplier/internal source material to onboard same-type parts into the controlled library pipeline, not for arbitrary user-uploaded parts.
...
- Expose descriptor draft intake through `inspectComponentDescriptorDraft`, `forge-tool descriptor-draft --descriptor-file <descriptor.json> --sources-file <sources.md>`, and `/api/workspaces/:workspaceId/components/draft-package` so a proposed Forge-controlled same-type part can be checked for `readyForLibraryPromotion` before it is added to the loaded descriptor library. Draft packages remain `readyForSelection: false` and `readyForReviewableGeneration: false` until they are actually loaded and selected through ProductPlan.
...
Implemented Controlled Prototype Readiness V1 foundation:

- Treat this as the next bounded loop after V3: decide whether a V3-generated ProductPlan can enter internal prototype build from Forge-controlled or vetted components. This is prototype readiness, not manufacturing readiness.
- Classify this first implementation as Core V1. It introduces `src/core/prototype_readiness.mjs` and a `prototype_readiness` generation job that runs for each ProductPlan revision after GeometrySpec/electronics layout.
- Introduce `ElectronicsDescriptor v1` as the controlled electronic fact source for current seed components. It tracks component id, MPN/proxy id, controlled supplier/source status, datasheet/spec source, internal measurement record, version, alternative/replacement relationship, trust level, review status, Forge approval, power budget, interfaces, connectors, and human-review flags.
- Derive an `electronics_descriptor_trust_report.json` for each revision. Core V1 lints selected ElectronicsDescriptors for required controlled evidence, blocks missing required evidence such as MPN/source/measurement/alternative records, and keeps evidence-complete reviewable parts as `Needs Review` rather than silently approved.
- Derive `ElectronicsSpec` from ProductPlan, ComponentDescriptor v2, ElectronicsDescriptor v1, and GeometrySpec. ProductPlan remains the product-intent source of truth, ComponentDescriptor remains the mechanical source of truth, ElectronicsDescriptor is the electronic fact source, and ElectronicsSpec is a derived result. `ElectronicsSpec` includes compact derived `powerPath` and `connectionRequirements` records so validation can connect electrical assumptions back to GeometrySpec route evidence.
- Run prototype-level electronics validation for obvious V1 conflicts: missing controller, missing power input, missing controller power route, rail current budget overflow, voltage/rail mismatch, unsupported interface assignment, GPIO exhaustion/conflict, interface connector-route mismatch, human-review components, and GeometrySpec cable-route linkage warnings.
- Derive a GeometrySpec-linked `AssemblyPlan` with ordered internal prototype build steps, sequence dependencies, route/feature/placement/access-volume references, risk items, manual-confirmation flags, and explicit boundaries against DFA, injection molding, robotic assembly, and production optimization. Core V1 blocks missing required assembly evidence such as required display retention/opening refs, route refs, or service/access refs before marking a prototype build-ready.
- Derive a development-board bring-up scaffold with pin map, interface map, module init stubs, test entrypoints, ProductPlan-derived behavior placeholders, checklist, and generated bring-up file contents (`pin_map.json`, `main.cpp`, `bringup_checklist.md`, and `behavior_rules.placeholder.json`). This is not OTA, device runtime, long-term user programming, or production firmware.
- Persist `electronics_descriptor_trust_report.json`, `electronics_spec.json`, `electronics_validation_report.json`, `assembly_plan.json`, `development_board_scaffold.json`, and `prototype_readiness_report.json` into each revision folder and summarize the compact status in revision manifest, revision ledger, and ContextPack without exposing raw electronics source evidence by default. `PrototypeReadinessReport.readinessGate` is the machine-readable V1 decision gate for `ready`, `needs_review`, or `blocked`, including component trust, ElectronicsSpec, electronics validation, AssemblyPlan, development-board scaffold, revision evidence, and boundary checks.
- Keep Future V2 out of this goal unless it directly proves prototype readiness. Custom PCB, manufacturing system, supplier ordering, OTA, full firmware runtime, arbitrary user component import, robotics, complex mechanisms, production certification, and frontend redesign stay out of scope.
