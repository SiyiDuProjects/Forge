...
- Mac client v1 uses SwiftUI `NavigationSplitView`, native `List(selection:)` project selection with right-click context menus, lower-left settings, native composer surfaces, spacing/radius tokens, Liquid Glass-compatible panel styling with material fallback, non-streaming Forge API calls for workspace restore / plan creation / chat turns, and a `WKWebView` bridge for the existing web preview. It hides the window title text and avoids app-specific top refresh/runtime/settings toolbar controls while preserving native macOS top-left window/sidebar controls; runtime/model selection stays in `Forge 设置`. It defaults to the same local Forge API and Codex runtime setting, can switch to the deterministic local Forge fallback, and only restores workspaces after `/api/health` succeeds.
