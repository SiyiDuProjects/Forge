...
- Frontend runtime selector in `Forge 设置` for `本地 Forge`, `Forge QueryEngine`, and `Codex`, with a center-thread SSE execution trace that renders ProductPlan creation, runtime/model request and response events, bounded Codex SDK event summaries, Forge tool progress, revision/proposal state, pending confirmations, and artifact generation status.
