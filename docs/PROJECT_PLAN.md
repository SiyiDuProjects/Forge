...
- Derive `ElectronicsSpec` from ProductPlan, ComponentDescriptor v2, ElectronicsDescriptor v1, and GeometrySpec. ProductPlan remains the product-intent source of truth, ComponentDescriptor remains the mechanical source of truth, ElectronicsDescriptor is the electronic fact source, and ElectronicsSpec is a derived result.
- Run prototype-level electronics validation for obvious V1 conflicts: missing controller, missing power input, rail current budget overflow, unsupported interface assignment, GPIO exhaustion/conflict, human-review components, and GeometrySpec cable-route linkage warnings.
