...
- Frontend runtime selector in `Forge 设置` for `本地 Forge`, `Forge QueryEngine`, and `Codex`, with a read-only runtime preflight status for Codex SDK availability and project thread state. The right inspector SSE execution status renders ProductPlan creation, runtime/model request and response events, bounded Codex SDK event summaries, Forge tool progress, revision/proposal state, pending confirmations, artifact generation status, and stop-current-turn behavior for long-running requests.
