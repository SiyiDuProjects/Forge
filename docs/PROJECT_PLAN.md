...
- Persist `electronics_descriptor_trust_report.json`, `electronics_spec.json`, `electronics_validation_report.json`, `assembly_plan.json`, `development_board_scaffold.json`, and `prototype_readiness_report.json` into each revision folder and summarize the compact status in revision manifest, revision ledger, and ContextPack without exposing raw electronics source evidence by default.
