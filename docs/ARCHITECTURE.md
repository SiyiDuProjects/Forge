...
- `app.js`: browser-side Codex-first conversation state machine, bilingual copy, ProductPlan rendering after tool-created plans, QueryEngine trace/confirmation UI, contact capture, 3D preview, and popover rendering.
...
- `src/core/product_conversation.mjs`: Codex-only blank/new-project conversation entry. It creates a conversation workspace and sends user messages to Codex; Forge does not provide backend-authored chat fallback or create ProductPlan state unless Codex calls `createProductPlan`.
...
- `src/core/runtime_plan_creation.mjs`: compatibility ProductPlan creation used by `/api/plans`; the web composer default for blank/new projects is `/api/conversations/turn/stream` so Codex decides whether to call `createProductPlan`.
- `src/core/guarded_files.mjs`: guarded-file snapshot and violation detector for Codex SDK turns. It blocks accidental direct edits to ProductPlan, manifests, revision sources, GeometrySpec, and artifacts unless the new Forge event type corresponds to the changed file class; event-only tools such as validation do not authorize direct ProductPlan or artifact writes. ProductPlan-level descriptor promotion and retirement are authorized only through `component_descriptor_promoted` and `component_descriptor_retired` events.
...
- `src/core/forge_actions.mjs`: stable action contract for future chat/tool-calling layers. It exposes summaries, component search, descriptor draft intake, workspace draft discovery, descriptor promotion/retirement, proposal staging, committed patch application, regeneration, validation, revert, and artifact retrieval while keeping ProductPlan and GeometrySpec under Forge control.
- `src/core/component_truth_registry.mjs`: standalone read-only ComponentDescriptor registry scanner. It reports schema validation, descriptor lint, `sourceEvidence`, `trustLevel`, `reviewStatus`, missing fields, common hardware module coverage, and no-mutation boundaries without selecting components or generating GeometrySpec.
- `src/core/revision_ledger.mjs`: pure project-level revision ledger builder. It summarizes ProductPlan revision records, ProductPlan-level component library entries, proposals, accepted/rejected patches, diff metadata, artifact manifests, and rollback events while marking GeometrySpec, validation, evidence, and GLB/STL/STEP files as derived outputs.
- `src/core/project_workspace.mjs`: file-backed Forge project runtime. It writes `data/workspaces/<planId>/project_manifest.json`, `runtime_plan.json`, `product_plan.json`, read-only `revision_ledger.json`, append-only `events.jsonl`, proposal JSON files, immutable revision folders, revision-scoped artifacts, local review files, generated `AGENTS.md`, `CURRENT_STATE.md`, `WORK_INDEX.md`, `DECISIONS.md`, `FORGE_TOOLS.md`, and project-local `skills/*.md`.
- `scripts/forge-tool.mjs`: CLI wrapper around Forge actions. Codex can run it from inside a project workspace to inspect summaries, search components, inspect descriptor package readiness, inspect workspace descriptor draft packages, promote descriptor drafts from files or `component-drafts/<draftId>/`, retire promoted descriptors, propose/stage/commit/apply changes, validate, generate, revert, fetch artifacts, submit local review material, or reject proposals without directly writing guarded files.
- `src/core/context_pack_builder.mjs`: compact context builder for future chat/runtime layers. It reads project folder summaries, ProductPlan component library summaries, and metadata while excluding raw GLB/STL/STEP bytes, promoted descriptor source text, and full event history.
...
8. The project folder runtime writes or updates `project_manifest.json`, `product_plan.json`, `revision_ledger.json`, append-only events, proposal files, and the revision folder for that locked state.
...
- Source of truth: `project_manifest.json` including optional `runtimeBinding`, `product_plan.json`, `events.jsonl`, `proposals/*.json`, `revisions/*/revision_manifest.json`, and `revisions/*/product_plan.json`. ProductPlan remains the authoritative product object.
- Derived project index: `revision_ledger.json` summarizes revision lineage, proposal decisions, accepted/rejected patches, diff metadata, artifact manifests, and rollback history for chat/runtime context.
- Derived generation inputs and evidence: `revisions/*/geometry-spec.json`, `revisions/*/component_descriptors.json`, validation reports, generation evidence reports, and artifact manifests are locked revision outputs derived from ProductPlan and ComponentDescriptor selections.
