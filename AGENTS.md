...
- `src/core/prototype_readiness.mjs`: Controlled Prototype Readiness V1 core; derives ElectronicsDescriptor trust reports, `ElectronicsSpec`, prototype-level electronics validation for voltage, power path, and connector-route alignment, GeometrySpec-linked `AssemblyPlan`, development-board bring-up scaffold, and `PrototypeReadinessReport` evidence without claiming PCB, manufacturing, OTA, full firmware runtime, certification, or arbitrary user component import.
