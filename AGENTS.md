...
- In the Mac client, do not render a custom center-thread title/status header such as `Forge`, current project title, runtime provider, or model status above the conversation. Let the system toolbar own top chrome and let the conversation content start directly under it.
