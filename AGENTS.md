...
- `src/core/product_conversation.mjs`: Codex-native blank/new-project conversation entry; creates a conversation workspace and sends all user messages to Codex. It does not create ProductPlan state unless Codex calls a Forge tool.
...
- `src/core/prototype_readiness.mjs`: Controlled Prototype Readiness V1 core; derives ElectronicsDescriptor trust reports, `ElectronicsSpec`, prototype-level electronics validation for voltage, power path, and connector-route alignment, GeometrySpec-linked `AssemblyPlan` feasibility checks, development-board bring-up config plus generated bring-up file contents, and `PrototypeReadinessReport` readiness-gate evidence without claiming PCB, manufacturing, OTA, full firmware runtime, certification, or arbitrary user component import.
...
- The send button sends user text to the Codex project thread first. Forge does not treat every message as ProductPlan input. ProductPlan creation/revision changes happen only when Codex calls Forge tools such as `createProductPlan`, `applyDesignPatch`, or `regenerateRevision`.
...
The current frontend should use Codex-native conversation as the default user-input entry. A blank/new project is a clean Codex conversation workspace, not a ProductPlan draft. It must not inherit demo prompts, default desktop-screen/frame content, old ProductPlan state, old GeometrySpec, or fallback plan data. If the server, API, or Codex runtime is unavailable, show a clear bilingual error and keep the draft input so the user can retry; do not synthesize a fake complete ProductPlan or Forge-authored chat response.
...
Codex-backed product turns use provider-neutral `runtimeBinding` persisted in `project_manifest.json`; legacy `codexThreadId` fields may be read for migration but new code must not write them. Codex owns all product conversation handling: ordinary chat, follow-up decisions, task splitting, method/tool/interface selection, and when to call Forge tools. Forge still owns ProductPlan persistence, revisions, events, GeometrySpec, artifacts, permissions, confirmation gates, guarded-file detection, and UI-ready outputs. Forge backend guardrails may deny unsafe or state-polluting tool calls, but Forge must not become a separate conversation brain. Do not use Codex runtime as a mode for editing Forge source code from within the product.
