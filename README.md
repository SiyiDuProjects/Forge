...
- Frontend runtime controls in `Forge 设置`: `本地 Forge`, `Forge QueryEngine`, and `Codex`, plus a center-thread streaming execution trace for ProductPlan creation, runtime/model requests, Forge tool progress, revision/proposal state, pending confirmation, and generated artifact status.
