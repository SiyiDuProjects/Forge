...
- Frontend runtime controls in `Forge 设置`: `本地 Forge`, `Forge QueryEngine`, and `Codex`, plus a center-thread execution trace that shows runtime/model response, Forge tool results, revision/proposal state, pending confirmation, and generated artifact status after each turn.
