...
- Frontend runtime controls in `Forge 设置`: `本地 Forge`, `Forge QueryEngine`, and `Codex`, plus a read-only runtime preflight status for Codex SDK availability and project thread state. The right inspector shows streaming execution status for ProductPlan creation, runtime/model requests, bounded Codex SDK event summaries, Forge tool progress, revision/proposal state, pending confirmation, generated artifact status, and a stop-current-turn control for long-running requests.
