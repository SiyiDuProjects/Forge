...
- ComponentDescriptor package readiness reports through Forge actions, `forge-tool component-package`, and the workspace component-package API so replacement parts can be reviewed before ProductPlan selection
- ComponentDescriptor draft intake reports through Forge actions, `forge-tool descriptor-draft`, and the workspace draft-package API so proposed new same-type parts can be checked for library-promotion readiness before becoming selectable ProductPlan inputs
- Workspace ComponentDescriptor draft discovery through `component-drafts/<draftId>/descriptor.json` plus `sources.md`, `forge-tool descriptor-drafts`, and workspace draft APIs so drop-in part packages can be checked before promotion
- Workspace ComponentDescriptor draft scaffolding through `forge-tool descriptor-scaffold` and workspace draft scaffold APIs so new same-type part packages start from a blocked, fillable `descriptor.json` plus `sources.md` skeleton instead of ad hoc JSON
