...
- Prototype readiness Core V1 under `src/core/prototype_readiness.mjs`: Forge-controlled ElectronicsDescriptor seed evidence and trust report, derived `ElectronicsSpec`, prototype-level electronics validation for voltage, power path, and connector-route alignment, GeometrySpec-linked `AssemblyPlan`, development-board bring-up scaffold, and `PrototypeReadinessReport` persisted with each ProductPlan revision
